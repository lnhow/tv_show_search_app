...
const SeriesInfo = (props) => {
  ...
  const seriesSummary =
    series.summary !== null ? series.summary : '<p>There is no summary yet</P>';
  console.log(series);

  return (
    <div className='container'>
      <img id='seriesImage' alt='Series banner' src={seriesImgUrl} />
      <div id='seriesInfo'>
        <h2 id='seriesName'>{series.name}</h2>
        <p id='seriesGenres'>
          <small>{seriesGenres}</small>
        </p>
        <p id='seriesPremiered'>
          Premiered: <b>{series.premiered}</b>
        </p>
        <p id='seriesRating'>
          Score: <b>{seriesRating}</b>
        </p>
        <div id='seriesSummary'>{parser(seriesSummary)}</div>
